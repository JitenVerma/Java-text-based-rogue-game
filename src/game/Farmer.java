...
public class Farmer extends Human {
	...
	public Action playTurn(Actions actions, Action lastAction, GameMap map, Display display) {
		...
		for(int i = 0;i < actions.size();i++) { //not sure how to add location part
			if (FertiliseNewAction.class.isInstance(actions.get(i)) && sowAction.class.isInstance(lastAction)) {
				return actions.get(i);
			}
			else if (HarvestAction.class.isInstance(actions.get(i)) && FertiliseNewAction.class.isInstance(lastAction)){
				return actions.get(i);
			}
			else if (sowAction.class.isInstance(actions.get(i))) {
				return actions.get(i);
